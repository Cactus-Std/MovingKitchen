...
2026-09-12 自动验收：100 项测试、TypeScript、ESLint、生产构建与 FaceX 模型检查通过；四个真实 Socket.IO 客户端可从同一版本并行提交，双菜板切菜不再互相阻塞。Chrome 四个独立浏览器上下文覆盖加入后分配工位、四位玩家并行控制、整份 Pizza、身份接管、刷新重连与重开。合成摄像头验证本地模型同时加载、录脸进度显示、采样中退出及摄像头释放。真人识别率尚未测量。
