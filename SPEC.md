...
- 香肠：切碎 → 放入烤箱。
- 芝士：切碎 → 放入烤箱。
- 面饼：放在菜板上，空手重复四次双手靠近再向两侧展开（STRETCH），进度达到 100% → 拿起并放入烤箱。

全部食材进入烤箱后开始烘烤。完成后使用隔热手套取出 Pizza，并使用披萨刀切开。

整合确认（2026-09-12）：保留手势模块 v0.2 的 STRETCH，每次有效展开增加 25%，面饼未达到 100% 时服务器拒绝入炉。当前演示每轮 5 分钟；四种食材齐全后自动烤制 20 秒，烤好后 15 秒内需取出，盘上的 Pizza 需要再切 5 次才进入结算；番茄、香肠和芝士在菜板也需要 5 次有效切割。时间和阈值集中在正式 shared/server 代码中。
