...
- 香肠：切碎 → 放入烤箱。
- 芝士：直接放入烤箱。
- 面饼：拿在手中，双手先靠近再向两侧展开（STRETCH）→ 放入烤箱。

全部食材进入烤箱后开始烘烤。完成后使用隔热手套取出 Pizza，并使用披萨刀切开。

整合确认（2026-09-12）：保留手势模块 v0.2 的 STRETCH，面饼未展开时服务器拒绝入炉。当前演示每轮 5 分钟、烤制 20 秒，烤好后 15 秒内需取出；菜板需要 5 次有效切割。时间和阈值集中在正式 shared/server 代码中。
